					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($k["s_id"]); ?></td>
								<td><?php echo (msubstr(strip_tags($k["s_content"]),0,120,'utf-8',true)); ?></td>
								<td><?php echo (getTime($k["s_time"])); ?></td>
								<td>
									<?php switch($k['s_view']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
										<?php case "1": ?><span class=" btn-dark-green btn-xs">显示</span><?php break;?>
										<?php default: ?>暂无数据<?php endswitch;?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('Say/sayEdit',array('id'=>$k['s_id']));?>">编辑</a>&nbsp;&nbsp;
...
$(document).ready(function(){
	$(".delsay",this).click(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Say/sayDel');?>",
			async:true,
			data:{"id":id},
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/anxun/blog/admin.php/Say/sayList.html");
				}else{
					err("错误!",data.msg);
