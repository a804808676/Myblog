...
<div id="content">
	<div id="content-header">
		<h1>相册评论列表</h1>
		<div class="btn-group">
			<a href="<?php echo U('Admin/Article/index');?>" class="btn btn-large" title="发表相册"><i class="fa fa-file"></i></a>
			<a href="<?php echo U('Admin/Article/articleList');?>" class="btn btn-large" title="相册列表"><i class="fa fa-comment"></i></a>
		</div>
	</div>
	<div id="breadcrumb">
					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($k["alc_id"]); ?></td>
								<td><?php echo ($k["alc_name"]); ?></td>
								<td><?php echo (msubstr(strip_tags(reFace($k["alc_content"])),0,120,'utf-8',true)); ?></td>
								<td><?php echo (getTime($k["alc_time"])); ?></td>
								<td>
									<?php if(($k['alc_rtime']) == "0"): ?><span class=" btn-dark-red btn-xs">未处理</span>
										<?php else: ?>
										<span class=" btn-dark-green btn-xs">已处理</span><?php endif; ?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('Admin/Album/albumContentEdit',array('id'=>$k['alc_id']));?>">编辑</a>&nbsp;&nbsp;
...
$(document).ready(function(){
	$(".delalbumcontent",this).click(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Album/albumContentDel');?>",
			async:true,
			data:{"id":id},
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/anxun/blog/admin.php/Album/albumContentList.html");
				}else{
					err("错误!",data.msg);
