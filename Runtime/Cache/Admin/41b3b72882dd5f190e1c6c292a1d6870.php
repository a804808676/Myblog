					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($k["g_id"]); ?></td>
								<td><?php echo ($k["g_name"]); ?></td>
								<td><?php echo (msubstr(strip_tags(reFace($k["g_content"])),0,120,'utf-8',true)); ?></td>
								<td><?php echo (getTime($k["g_time"])); ?></td>
								<td>
									<?php if(($k['g_rtime']) == "0"): ?><span class=" btn-dark-red btn-xs">未处理</span>
										<?php else: ?>
										<span class=" btn-dark-green btn-xs">已处理</span><?php endif; ?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('Admin/Gust/gustEdit',array('id'=>$k['g_id']));?>">编辑</a>&nbsp;&nbsp;
...
$(document).ready(function(){
	$(".delgust",this).click(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Gust/gustDel');?>",
			async:true,
			data:{"id":id},
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/anxun/blog/admin.php/Gust/gustList.html");
				}else{
					err("错误!",data.msg);
