					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('Article/articleEdit',array('id'=>$k['a_id']));?>">编辑</a>&nbsp;&nbsp;
...
$(document).ready(function(){
	$(".delsay",this).click(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Article/articleDel');?>",
			async:true,
			data:{"id":id},
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/anxun/blog/admin.php/Article/articleList.html");
				}else{
					err("错误!",data.msg);
