...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="回到首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a>
		<a href="<?php echo U('Index/index');?>" class="current">仪表盘</a>
	</div>
		...
		<div class="row">
			<div class="col-xs-12 center" style="text-align: center;">					
				<ul class="quick-actions">
					...
					<li><a href="<?php echo U('System/email');?>"><i class="icon-mail"></i>邮件设置</a></li>
					<li><a href="<?php echo U('User/userList');?>"><i class="icon-user"></i>用户管理</a></li>
					<li><a href="<?php echo U('Say/index');?>"><i class="icon-survey"></i>写说说</a></li>
					<li><a href="<?php echo U('Article/index');?>"><i class="icon-survey"></i>写文章</a></li>
					<li><a href="<?php echo U('Down/index');?>"><i class="icon-download"></i>下载统计</a></li>
		...
		<div class="row">
			<div class="col-xs-12">
				<div class="alert alert-info">
					<i class="fa fa-code"></i>  欢迎使用  <strong>沈耀国博客</strong>  后台管理程序. 请遵循使用守则，谢谢!
				...
				<div class="widget-box">
					...
					<div class="widget-content">
						<div class="row">
							...
							<div class="col-xs-12 col-sm-4">
								<ul class="site-stats">
									<li><div class="cc">QQ群 ： 455466010</div></li>
					...
					<div class="widget-content nopadding">
						...
						<?php if(is_array($s_content)): $i = 0; $__LIST__ = $s_content;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo["ac_id"])): ?><li>
		        				...
		        				<span class="user-info"> By: <?php echo ($vo["ac_name"]); ?> on <?php echo (getTime($vo["ac_time"])); ?>, IP：<?php echo ($vo["ac_ip"]); ?> 在文章中评论</span>
		          				...
		          				<?php else: ?>
		          				<a href="<?php echo U('Article/articleContentEdit',array('id'=>$vo['ac_id']));?>" class="btn btn-primary btn-xs">回复</a><?php endif; ?>
		          				 <a href="<?php echo U('Article/articleContentList');?>" class="btn btn-danger btn-xs">删除</a>
		          				</div>
		        			</li><?php endif; ?>
			        		<?php if(!empty($vo["alc_id"])): ?><li>
		        				...
		        				<span class="user-info"> By: <?php echo ($vo["alc_name"]); ?> on <?php echo (getTime($vo["alc_time"])); ?>, IP：<?php echo ($vo["alc_ip"]); ?> 在相册中评论</span>
		          				...
		          				<?php else: ?>
		          				<a href="<?php echo U('Album/albumContentEdit',array('id'=>$vo['alc_id']));?>" class="btn btn-primary btn-xs">回复</a><?php endif; ?>
		          				<a href="<?php echo U('Album/albumContentList');?>" class="btn btn-danger btn-xs">删除</a>
		          				</div>
		          				</li><?php endif; ?>
			        		<?php if(!empty($vo["sc_id"])): ?><li>
		        				...
		        				<span class="user-info"> By: <?php echo ($vo["sc_name"]); ?> on <?php echo (getTime($vo["sc_time"])); ?>, IP：<?php echo ($vo["sc_ip"]); ?> 在相册中评论</span>
		          				...
		          				<?php else: ?>
		          				<a href="<?php echo U('Say/sayContentEdit',array('id'=>$vo['sc_id']));?>" class="btn btn-primary btn-xs">回复</a><?php endif; ?>
		          				<a href="<?php echo U('Say/sayContentList');?>" class="btn btn-danger btn-xs">删除</a>
		          				</div>
							...
							<?php if(is_array($gusts)): $i = 0; $__LIST__ = $gusts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><li>
								...
								<div class="comments">
									...
									<?php if(($g["g_rtime"]) != "0"): ?><a class="btn btn-primary btn-xs">已回复</a>
		          					<?php else: ?>
		          						<a href="<?php echo U('Gust/gustEdit',array('id'=>$g['g_id']));?>" class="btn btn-primary btn-xs">回复</a><?php endif; ?>
									<a href="<?php echo U('Gust/gustList');?>" class="btn btn-danger btn-xs">删除</a>
								</div>
							</li><?php endforeach; endif; else: echo "" ;endif; ?>
							<li class="viewall">
								<a title="查看全部" class="tip-top" href="<?php echo U('Gust/gustList');?>"> + 查看全部 + </a>
							</li>
						</ul>
					</div>
				</div>
